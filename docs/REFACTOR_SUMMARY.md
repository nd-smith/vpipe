...
```
pipeline/plugins/itel_cabinet_api/
...
enrichment_handlers:
  ...
  - type: validation
    config:
      required_fields: [...]
      field_rules: {...}
  - type: pipeline.plugins.itel_cabinet_api.handlers.conditional_completed:ConditionalCompletedHandler
    config:
      wrapped_handler:
        type: lookup
        config: {...}
...
**The old enrichment handler framework still exists** in `pipeline/plugins/shared/enrichment.py`.
