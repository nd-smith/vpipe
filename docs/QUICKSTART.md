...
Dead Letter Queues capture messages that fail processing after all retry attempts are exhausted. The DLQ CLI tools allow you to review, replay, or resolve these failed messages.

### Verisk (XACT) DLQ CLI

The Verisk pipeline has a single DLQ for download failures.

#### List DLQ Messages

View all messages in the DLQ with summary information:

```bash
cd src
python -m pipeline.verisk.dlq.cli list
```
...
View detailed information for a specific message by trace_id:

```bash
python -m pipeline.verisk.dlq.cli view <trace_id>
```

**Shows:**
- Full original task details
- Complete error history
- Retry metadata
- Message offset and partition

#### Replay Message to Pending Topic

Send a failed message back to the pending topic for reprocessing (resets retry_count to 0):

```bash
python -m pipeline.verisk.dlq.cli replay <trace_id>
```

**What happens:**
1. Original task is extracted from DLQ message
2. retry_count is reset to 0
3. Message is sent to `downloads_pending` topic
4. Metadata includes `replayed_from_dlq: true`
5. DLQ offset is committed (message removed from DLQ)

#### Resolve Message (Acknowledge Without Replay)

Mark a message as resolved without replaying it:

```bash
python -m pipeline.verisk.dlq.cli resolve <trace_id>
```

Use this when:
- The failure is expected and can be ignored
- The issue was fixed manually outside the pipeline
- The message should not be reprocessed

### ClaimX DLQ CLI

The ClaimX pipeline has TWO DLQ types: download and enrichment. All commands require the `--type` flag.

#### List DLQ Messages

List download DLQ messages:

```bash
cd src
python -m pipeline.claimx.dlq.cli list --type download
```

List enrichment DLQ messages:

```bash
python -m pipeline.claimx.dlq.cli list --type enrichment
```
...
View download DLQ message by media_id:

```bash
python -m pipeline.claimx.dlq.cli view --type download <media_id>
```

View enrichment DLQ message by event_id:

```bash
python -m pipeline.claimx.dlq.cli view --type enrichment <event_id>
```

#### Replay Message to Pending Topic

Replay download DLQ message:

```bash
python -m pipeline.claimx.dlq.cli replay --type download <media_id>
```

Replay enrichment DLQ message:

```bash
python -m pipeline.claimx.dlq.cli replay --type enrichment <event_id>
```

**What happens:**
1. Original task is extracted from DLQ message
2. retry_count is reset to 0
3. Message is sent to appropriate pending topic:
   - Download: `downloads_pending`
...
Resolve download DLQ message:

```bash
python -m pipeline.claimx.dlq.cli resolve --type download <media_id>
```

Resolve enrichment DLQ message:

```bash
python -m pipeline.claimx.dlq.cli resolve --type enrichment <event_id>
```

### Common DLQ Workflows

#### Workflow 1: Review and Replay All Failed Messages
...
# Review specific messages
python -m pipeline.verisk.dlq.cli view trace-123

# Replay fixable messages
python -m pipeline.verisk.dlq.cli replay trace-123

# Resolve unfixable messages
python -m pipeline.verisk.dlq.cli resolve trace-456
```

#### Workflow 2: Bulk Replay After Fix

After fixing a systemic issue (e.g., API endpoint restored):

```bash
# List messages to get IDs
cd src
python -m pipeline.claimx.dlq.cli list --type download > dlq_messages.txt

# Replay each message (manual or scripted)
python -m pipeline.claimx.dlq.cli replay --type download media_001
python -m pipeline.claimx.dlq.cli replay --type download media_002
# ... etc
```

#### Workflow 3: Investigate Failure Patterns

```bash
# List all download DLQ messages
cd src
python -m pipeline.claimx.dlq.cli list --type download

# View details for messages with same error_category
python -m pipeline.claimx.dlq.cli view --type download media_123
python -m pipeline.claimx.dlq.cli view --type download media_456

# Identify common patterns (URL issues, auth failures, etc.)
```

### DLQ Best Practices

1. **Regular Monitoring**: Check DLQ daily for failed messages
2. **Root Cause Analysis**: Investigate error patterns before mass replay
3. **Fix Systemic Issues**: Address infrastructure problems before replaying
4. **Document Resolutions**: Track why messages were resolved without replay
5. **Replay vs Resolve**: Only replay if the underlying issue is fixed

### DLQ Message Flow

```
Normal Processing:
Event → Pending Topic → Worker → Retry (up to 4 times) → Success

DLQ Flow:
Event → Pending Topic → Worker → Retry (4 failures) → DLQ Topic

Manual Recovery:
DLQ Topic → [CLI Review] → Replay → Pending Topic → Worker → Success
                        ↘ Resolve → [Acknowledged, removed from DLQ]
```

### Retry Configuration

All pipelines use the same retry delays (configured in `config/config.yaml`):

```yaml
retry_delays: [300, 600, 1200, 2400]  # 5m, 10m, 20m, 40m
```

After 4 failed retries (approximately 70 minutes total), messages are sent to the DLQ for manual review.
