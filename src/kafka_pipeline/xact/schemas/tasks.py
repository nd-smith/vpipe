...
class XACTEnrichmentTask(BaseModel):
    ...
    event_id: str = Field(
        ..., description="Unique event identifier generated by event ingester", min_length=1
    )
    trace_id: str = Field(
        ..., description="Original trace ID from the source event (for correlation)", min_length=1
    )
    event_type: str = Field(..., description="Type of event (e.g., 'xact')", min_length=1)
...
class DownloadTaskMessage(BaseModel):
    ...
    event_type: str = Field(
        ..., description="Type of the originating event (e.g., 'xact')", min_length=1
    )
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional context passed through from original event"
    )
