...
Usage:
    ...
    python -m pipeline.simulation.dummy_producer --domains claimx --max-events 100
...
Examples:
    ...
    SIMULATION_MODE=true python -m pipeline.simulation.dummy_producer --domains claimx --max-events 100

    # Generate events at 60/minute for both domains
    SIMULATION_MODE=true python -m pipeline.simulation.dummy_producer --domains claimx,xact --events-per-minute 60

    # iTel Cabinet workflow testing
    SIMULATION_MODE=true python -m pipeline.simulation.dummy_producer --plugin-profile itel_cabinet_api --max-events 20

    # Burst mode for load testing
    SIMULATION_MODE=true python -m pipeline.simulation.dummy_producer --burst-mode --burst-size 100
...
async def main():
    ...
    from pipeline.common.dummy.file_server import FileServerConfig
    from pipeline.common.dummy.generators import GeneratorConfig
    from pipeline.common.dummy.source import DummyDataSource, DummySourceConfig
