    ...
    async def _consume_loop(self) -> None:
        ...
        while self._running:
            ...
            try:
                ...
                if not is_retryable:
                    logger.error(
                        "Non-retryable error in receive loop, not reconnecting",
                        extra={
                            "error_type": type(e).__name__,
                            "error": str(e),
                            "error_category": classified.category.value,
                ...
                logger.warning(
                    "Transient error in receive loop, reconnecting in %.1fs "
                    "(attempt %d/%d): %s: %s",
                    delay,
                    consecutive_errors,
                    max_consecutive_errors,
                ...
                try:
                    ...
                    logger.debug("Error closing consumer during reconnect", exc_info=True)
                ...
                logger.info(
                    "Reconnected to Event Hub, resuming consumption",
                    extra={
                        "entity": self.eventhub_name,
                        "consumer_group": self.consumer_group,
                        "attempt": consecutive_errors,
