...
class TestCreateConsumerCheckpointStore:
    ...
    def reset_eventhub_config(self):
        ...
        from kafka_pipeline.common.transport import reset_eventhub_config
    ...
    async def test_create_consumer_eventhub_gets_checkpoint_store(self):
        ...
        from kafka_pipeline.common.transport import create_consumer, TransportType
        ...
        with patch("kafka_pipeline.common.transport.get_checkpoint_store", new_callable=AsyncMock) as mock_get_checkpoint:
            mock_get_checkpoint.return_value = mock_checkpoint_store

            # Mock EventHubConsumer constructor
            with patch("kafka_pipeline.common.eventhub.consumer.EventHubConsumer") as mock_consumer_class:
    ...
    async def test_create_consumer_kafka_does_not_use_checkpoint_store(self):
        ...
        with patch("kafka_pipeline.common.transport.get_checkpoint_store", new_callable=AsyncMock) as mock_get_checkpoint:
            with patch("kafka_pipeline.common.consumer.BaseKafkaConsumer") as mock_consumer_class:
    ...
    async def test_create_consumer_with_topic_key_resolves_eventhub_name(self):
        ...
        mock_eventhub_config = {
            "xact": {
                "events": {
                    "eventhub_name": "com.allstate.pcesdopodappv1.xact.events.raw",
        ...
        with patch("kafka_pipeline.common.transport.get_checkpoint_store", new_callable=AsyncMock) as mock_get_checkpoint:
            ...
            with patch("kafka_pipeline.common.transport._load_eventhub_config") as mock_load_config:
        ...
        mock_consumer_class.assert_called_once()
        call_kwargs = mock_consumer_class.call_args.kwargs
        assert call_kwargs["eventhub_name"] == "com.allstate.pcesdopodappv1.xact.events.raw"
