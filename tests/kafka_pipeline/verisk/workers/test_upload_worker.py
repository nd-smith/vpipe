...
from kafka_pipeline.verisk.workers.upload_worker import UploadWorker, UploadResult
...
def kafka_config():
    ...
    return KafkaConfig(
        ...
        onelake_domain_paths={"verisk": "abfss://test@test.dfs.core.windows.net/Files"},
...
def sample_cached_message():
    ...
    return CachedDownloadMessage(
        ...
        metadata={"source_partition": 3, "source_system": "verisk"},
    )
...
class TestUploadWorkerInitialization:
    ...
    async def test_initialization_success(self, kafka_config):
        ...
        assert worker.topic == kafka_config.get_topic("verisk", "downloads_cached")
...
class TestUploadWorkerProcessing:
    ...
    async def test_process_single_upload_success(
        ...
        worker.onelake_clients = {"verisk": mock_client}
        ...
        assert call_args.kwargs["topic"] == kafka_config.get_topic("verisk", "downloads_results")
...
class TestUploadWorkerGracefulShutdown:
    ...
    async def test_stop_waits_for_in_flight_uploads(self, kafka_config):
        ...
        worker._consumer = AsyncMock()
        worker.producer = AsyncMock()
        worker.onelake_clients = {"verisk": AsyncMock()}
