...
class TestBaseKafkaConsumerInit:
    ...
    def test_init_creates_default_circuit_breaker(
        ...
        with patch("pipeline.common.consumer.get_circuit_breaker") as mock_get_breaker:
...
class TestBaseKafkaConsumerStartStop:
    ...
    async def test_start_creates_consumer(
        ...
        with patch(
            "pipeline.common.consumer.AIOKafkaConsumer", return_value=mock_aiokafka_consumer
        ):
            with patch("pipeline.common.consumer.create_kafka_oauth_callback") as mock_oauth:
